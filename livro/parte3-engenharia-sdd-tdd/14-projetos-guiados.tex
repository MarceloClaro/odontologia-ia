...
\subsection{Implementar um Classificador de Cáries com CNN}
Você aplicará SDD+TDD a um problema real de classificação de cáries proximais em radiografias bitewing.

\subsection{Construir um Segmentador de Dentes com U-Net}
Exploraremos a especificação, implementação e validação de um modelo de segmentação semântica para radiografias panorâmicas.

\subsection{Desenvolver um Detector de Perda Óssea Periodontal}
Você integrará CNNs e métricas clínicas para quantificar perda óssea alveolar automaticamente.

\subsection{Executar Notebooks no Google Colab com Reprodutibilidade}
Ao final, todos os projetos estarão disponíveis como notebooks executáveis, prontos para adaptação aos seus próprios dados.
...
\subsubsection{Backbone EfficientNet-B3 com transfer learning}
...
\subsubsection{Hiperparâmetros de treinamento}
...
\subsubsection{Detecção de landmarks e medição calibrada}
...
\subsubsection{Resultados clínicos}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lccX}
    ...
    \textbf{Métrica} & \textbf{Valor} & \textbf{Threshold} & \textbf{Status} \\
    \midrule
    MAE (mm) & 0,42 & $<$ 0,50 & \textcolor{corTesteSucesso}{Aprovado} \\
    Pearson $r$ & 0,967 & $>$ 0,95 & \textcolor{corTesteSucesso}{Aprovado} \\
    Concordância estágio & 88,5\% & $>$ 85\% & \textcolor{corTesteSucesso}{Aprovado} \\
...
\subsubsection{Adaptações para o domínio odontológico}
