...
\subsection{Compreender o Ciclo SDD: Spec, Implement, Verify, Deploy}
Você aprenderá a disciplina de nunca implementar sem antes especificar, compreendendo como especificações executáveis reduzem defeitos em software odontológico.

\subsection{Escrever Especificações com Critérios de Aceitação Mensuráveis}
Dominará a estrutura Problema--Entrada--Saída--Critérios, criando especificações verificáveis para classificação, segmentação e detecção.

\subsection{Diferenciar SDD, TDD e BDD}
Você entenderá como estas três metodologias se complementam no desenvolvimento de sistemas de IA clínica.

\subsection{Aplicar o Decorator \texttt{@spec} do Pacote OdontoIA}
Ao final, você utilizará ferramentas práticas para rastrear especificações no código, garantindo rastreabilidade bidirecional.
...
\subsubsection{O paralelo com a aviação civil}
...
\indice{DO-178C} \indice{segurança crítica}

\subsubsection{Comparação de criticidade entre domínios}
...
\subsubsection{A crise de reprodutibilidade na IA odontológica}
...
\subsubsection{Como o SDD garante reprodutibilidade}
...
\subsubsection{Exemplo de especificação vaga}
...
\subsubsection{Exemplo de especificação precisa}
...
\subsubsection{API de verificação programática}
...
\subsubsection{Exemplos de verificação}
