...
\subsection{Compreender a Crise de Reprodutibilidade na IA Odontológica}
Você analisará os fatores que tornam a reprodução de experimentos desafiadora: datasets privados, dependências conflitantes e falta de padronização.

\subsection{Escolher a Licença Correta para Código e Modelos}
Dominará as diferenças entre MIT, Apache, GPL e CC BY, sabendo qual usar em cada contexto de pesquisa e publicação.

\subsection{Criar Model Cards e Publicar no Hugging Face}
Você aprenderá a documentar e compartilhar modelos com a comunidade, seguindo os padrões internacionais de transparência.

\subsection{Versionar Dados com DVC e Publicar no Zenodo}
Ao final, você implementará um fluxo completo de publicação reproduzível, com DOI, ambiente Docker e dataset versionado.

\section{Reprodutibilidade Computacional em Odontologia}

\subsection{A crise de reprodutibilidade}

\subsubsection{O problema da reprodutibilidade na ciência}

Em 2016, a revista \textit{Nature} publicou uma pesquisa histórica:
\textbf{mais de 70\% dos cientistas já tentaram — e falharam — em
reproduzir experimentos de outros pesquisadores}.
...
\subsubsection{Fatores agravantes na IA odontológica}

Na IA odontológica, o problema é agravado por três fatores:
...
\subsubsection{Código, ambiente e dataset}
...
\subsubsection{Scripts e métricas}
...
\subsubsection{Para código e datasets}
...
\subsubsection{Para modelos e documentação}
...
\subsubsection{Caso 1: O Perigo do Copy-Paste sem Licença}
...
\subsubsection{Caso 2: Dataset ``Anonimizado'' que Não Era}
