...
\subsection{Compreender a Operação de Convolução 2D}
Você aprenderá como kernels extraem padrões locais de imagens, desde bordas simples até texturas complexas em radiografias odontológicas.

\subsection{Dominar Arquiteturas CNN Clássicas}
Exploraremos LeNet, AlexNet, VGG, ResNet e EfficientNet, entendendo suas inovações e aplicações em diagnóstico por imagem.

\subsection{Aplicar Transfer Learning em Cenários de Dados Escassos}
Você dominará o pipeline de fine-tuning de modelos pré-treinados, essencial quando o dataset clínico tem poucas centenas de imagens.

\subsection{Configurar Aumento de Dados Odontológico}
Aprenderá as técnicas de augmentação seguras para radiografias, evitando distorções que comprometeriam a validade clínica.
...
\subsubsection{Conexões Residuais}
...
\subsubsection{Variantes ResNet}
...
\subsubsection{Comparação de Arquiteturas}
