...
O conceito de gêmeo digital representa o horizonte mais ambicioso da odontologia digital: a criação de uma réplica virtual dinâmica do paciente que integra dados de múltiplas fontes --- CBCT, scanner intraoral, fotografia facial, sensores --- e utiliza inteligência artificial para simular intervenções, predizer desfechos e otimizar decisões clínicas em tempo real. Diferentemente de modelos 3D estáticos ou simulações isoladas, o gêmeo digital incorpora a dimensão temporal e a retroalimentação contínua de dados clínicos, criando um ciclo virtuoso de aprendizado e refinamento. A citação que abre este capítulo estabelece esta distinção conceitual fundamental e nos convida a explorar como esta tecnologia está sendo aplicada ao planejamento de implantes, cirurgia ortognática, periodontia e reabilitação oral completa.

\subsection{Objetivos de Aprendizagem}

Ao final deste capítulo, você será capaz de:

\subsubsection{Diferenciar Modelo 3D, Simulação e Gêmeo Digital}
Você compreenderá as distinções conceituais fundamentais entre estas três abordagens, essenciais para o planejamento digital em odontologia.

\subsubsection{Integrar CBCT, Scanner Intraoral e Modelos de IA}
Exploraremos a fusão de múltiplas modalidades de imagem para criar réplicas virtuais completas do paciente.

\subsubsection{Aplicar Gêmeos Digitais ao Planejamento de Implantes e Cirurgia Ortognática}
Você aprenderá como gêmeos digitais permitem simular intervenções antes de realizá-las no paciente real.

\subsubsection{Implementar um Gêmeo Digital com o Módulo \texttt{odontoia.models.digital\_twin}}
Ao final, você construirá um protótipo funcional de gêmeo digital odontológico integrando segmentação, FEM e predição de desfechos.
...
\subsubsection{Modalidades de Aquisição}\indice{CBCT}\indice{scanner intraoral}\indice{fotografia facial 3D}
...
\subsubsection{Método de Fusão Coroa-Raiz}\indice{fusão de imagens}\indice{PointNet++}\indice{coroa-raiz}
...
\subsubsection{Fluxo do Gêmeo Digital para Implantes}\indice{gêmeo digital}\indice{Ahn 2024}\indice{simulação FEM}
...
\subsubsection{Abordagem Causal e Contrafactuais}\indice{CaTGO}\indice{DAG}\indice{simulação contrafactual}
...
O gargalo tradicional do FEM é o tempo computacional: uma simulação mastigatória completa (vários ciclos de carga com diferentes direções e magnitudes) pode levar horas de processamento.

\subsubsection{Estratégias de Aceleração}\indice{FEM}\indice{surrogate models}\indice{PINNs}

A IA oferece duas estratégias de aceleração:
...
A próxima evolução dos gêmeos digitais odontológicos envolve a integração com dispositivos de Internet das Coisas (IoT) e sensores vestíveis que alimentam o modelo com dados contínuos --- não apenas pontuais (consultas).

\subsubsection{Dispositivos Conectados e Sensores}\indice{IoT}\indice{sensores vestíveis}\indice{escovas inteligentes}
